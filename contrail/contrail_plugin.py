"""Contrail — main plugin class.

A dock panel to pick a provider and an area (current map view or a drawn box),
start/stop a live ADS-B feed, and watch aircraft move. Polling happens inside
the provider; the map is updated in batches by a timer so QGIS stays responsive.
"""
...
# Drop aircraft not heard from in this many seconds. Long enough to ride out a
# gap between reports, short enough that the count tracks "present now".
STALE_SECONDS = 240
...
class ContrailPlugin:
    ...
    def initGui(self):
        ...
        self.action = QAction(icon, "Contrail", self.iface.mainWindow())
        self.action.setCheckable(True)
        self.action.toggled.connect(self._toggle_dock)
        self.iface.addToolBarIcon(self.action)
        self.iface.addPluginToMenu("Contrail", self.action)
        # re-query the new view when the map moves (debounced)
    ...
    def unload(self):
        ...
        if self.action is not None:
            ...
            self.iface.removePluginMenu("Contrail", self.action)
            self.action = None
    ...
    def _build_dock(self):
        ...
        intro = QLabel(
            "Watch live aircraft traffic. Pick an area, then Start — planes "
            "stream onto the map and move in real time."
        )
        ...
        self.chk_airborne = QCheckBox("Show only airborne aircraft")
        self.chk_airborne.setToolTip(
            "Hide aircraft reporting on the ground, showing only those airborne.")
        self.chk_airborne.toggled.connect(self._on_airborne_toggled)
    ...
    def _on_airborne_toggled(self, checked):
        self.store.set_airborne_filter(checked)
    ...
    def _start(self):
        ...
        if not is_configured(cls):
            bar.pushWarning("Contrail", f"Configure {cls.label} first.")
            ProviderConfigDialog(cls, self.iface.mainWindow()).exec()
            if not is_configured(cls):
                return
        ...
        if bbox is None:
            bar.pushWarning("Contrail", "Draw an area on the map first.")
            return

        dbg(f"Started tracking with {cls.label}.")
        self.store.ensure_layer()
        self.provider = cls(load_settings(cls))
        self.provider.aircraft_update.connect(self.store.ingest)
    ...
    def _stop(self):
        ...
        if self.provider is not None:
            try:
                ...
                QgsMessageLog.logMessage(f"stop: {exc}", "Contrail", Qgis.MessageLevel.Warning)
            self.provider = None
        self.store.flush()
        self._running = False
    ...
    def _refresh_area(self):
        ...
        if bbox is not None:
            ...
            self.store.retain_within(bbox)  # drop aircraft outside the new view

    def _on_tick(self):
        try:
            self.store.flush()
            # Clip to the area being watched every tick, not just on pan/zoom, so
            # the layer never holds aircraft outside the current view and the
            # count stays honest to the frame.
            bbox = self._bbox_wgs84()
            if bbox is not None:
    ...
    def _dismiss_stale_highlights(self):
        ...
        clicked and QGIS never moves it, so on a moving aircraft it lingers at
        the wrong spot. Hide our layer's highlights (non-destructively) once
    ...
    def _update_status(self):
        ...
        if self.chk_airborne is not None and self.chk_airborne.isChecked():
            # filter is on — report what's actually shown vs. the total tracked
            self.lbl_status.setText(
                f"{self._last_status} · {self.store.airborne_count()} airborne "
                f"of {total} aircraft")
        else:
            self.lbl_status.setText(f"{self._last_status} · {total} aircraft")
